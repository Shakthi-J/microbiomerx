...
// ─────────────────────────────────────────────────────────────────
function ItemCard({
  item, isFlashing, onToggle, onNoteChange, onDetailChange,
  onLabelChange, onConfirmNew, onDiscard, disabled,
}: {
  item: EditableItem
  isFlashing: boolean
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between pt-1">
          <button type="button" onClick={() => onDiscard(item.key)} className="text-xs text-red-400 hover:text-red-600 transition-colors">✕ Discard</button>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.rationale && (
              <p className="mt-1.5 text-xs text-slate-400 leading-relaxed border-l-2 border-slate-200 pl-2">{item.rationale}</p>
            )}
      ...
      <div className="mt-3 border-t border-slate-100 pt-3">
        {editingNote ? (
          <textarea autoFocus
            ...
            rows={2} placeholder="Add doctor note…" value={item.doctorNote}
...
function Section({ title, icon, count, children, onAddItem, disabled }: {
  title: string; icon: string; count: number; children: React.ReactNode; onAddItem: () => void; disabled?: boolean
}) {
  return (
  ...
  useEffect(() => {
    async function load() {
      ...
      setReport({
        ...
        rych_tier:       Number(ro?.rych_tier ?? 1),
        rych_tier_label: ro?.rych_tier_label ?? 'Unknown',
        marker_count:    Number(ro?.marker_count ?? 0),
        conditions_flagged:      (ro?.conditions_flagged      ?? []) as string[],
        contraindication_alerts: (ro?.contraindication_alerts ?? []) as any[],
      })

      // 2. Build base sections from rules_output
      ...
      if (ro) {
        baseSections = {
          supplements: (ro.supplements ?? []).map((s: any, i: number) => ({
            ...
            rationale: s.mechanism, doctorNote: '', status: 'kb' as ItemStatus,
            category: s.aic_category, phase: s.protocol_phase, contraindications: '',
          })),
      ...
      // 3. Load saved prescription — overrides base sections if present
      ...
      // 4. Merge AIC recs AFTER prescription load (so they aren't overwritten)
      const normalisePhase = (p: string) => {
        const m = (p || '').match(/Phase\s*(\d+(\+\d+)?)/i)
        return m ? `Phase ${m[1]}` : 'Phase 2'
      }

      let aicData = rep.aic_supplement_recommendations as any

// ── Auto-generate AIC if not cached ──────────────────────────
if (!aicData && ro) {
  try {
    const aicRes = await fetch('/api/aic-supplements', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ report_id: reportId, report_data: ro }),
    })
    if (aicRes.ok) {
      const aicJson = await aicRes.json()
      aicData = aicJson
    }
  } catch (e) {
    console.warn('AIC generation failed silently:', e)
  }
}
...
if (aicData) {
        const aicAll: any[] = [
          ...(aicData.phase1                   ?? []),
          ...(aicData.phase2_infection_control ?? []),
          ...(aicData.phase2_probiotics        ?? []),
          ...(aicData.phase2_nutrition         ?? []),
          ...(aicData.phase3                   ?? []),
        ]

        const existingAicNames = new Set(
          baseSections.supplements.map(s => s.aicProduct?.toLowerCase().trim()).filter(Boolean)
        )
        const existingLabels = new Set(
          baseSections.supplements.map(s => s.label?.toLowerCase().trim())
        )

        console.log('AIC sample:', JSON.stringify(aicAll[0], null, 2))
        const aicItems: EditableItem[] = aicAll
          .filter(r => {
            const aicName  = (r.aic_product_name || r.product_name || '').toLowerCase().trim()
            const prodName = (r.product_name || '').toLowerCase().trim()
            return !existingAicNames.has(aicName) && !existingLabels.has(prodName)
          })
          .map((r, i) => ({
            key:        `aic_${r.product_key || i}`,
            label:      r.product?.name || r.product_key || '',
            aicProduct:  '',
            detail:     [r.product?.dose, r.product?.timing].filter(Boolean).join(' · '),
            rationale:  r.ai_rationale || r.product?.subtitle || '',
            doctorNote: '',
            status:     'kb' as ItemStatus,
            source:     'aic' as const,
            category:   r.product?.category || '',
            phase:      normalisePhase(
              r.product?.phase ? `Phase ${r.product.phase}` : ''
            ),
            contraindications: '',
          }))
        ...
        baseSections = {
          ...baseSections,
          supplements: [...baseSections.supplements, ...aicItems],
        }
      }

      // 5. Set final merged sections
  ...
  const updateNote = useCallback((section: keyof ReviewSections, key: string, note: string) => {
    if (key.endsWith('__rationale')) {
      ...
      setSections(prev => ({ ...prev, [section]: prev[section].map(item => item.key === realKey ? { ...item, rationale: note } : item) }))
      return
    }
    setSections(prev => ({
      ...prev,
      [section]: prev[section].map(item =>
        item.key === key ? { ...item, doctorNote: note, status: item.status === 'kb' ? 'modified' : item.status } : item
      ),
    }))
  }, [])
  ...
  const updateDetail = useCallback((section: keyof ReviewSections, key: string, detail: string) => {
    setSections(prev => ({
      ...prev,
      [section]: prev[section].map(item =>
        item.key === key ? { ...item, detail, status: item.status === 'kb' ? 'modified' : item.status } : item
      ),
    }))
  }, [])
  ...
  const updateLabel = useCallback((section: keyof ReviewSections, key: string, value: string) => {
    if (key.endsWith('__aic')) {
      const rk = key.replace('__aic', '')
      setSections(prev => ({ ...prev, [section]: prev[section].map(item => item.key === rk ? { ...item, aicProduct: value } : item) }))
      return
    }
    if (key.endsWith('__phase')) {
      const rk = key.replace('__phase', '')
      setSections(prev => ({ ...prev, [section]: prev[section].map(item => item.key === rk ? { ...item, phase: value } : item) }))
      return
    }
  ...
  const addItem = useCallback((section: keyof ReviewSections) => {
    ...
    setSections(prev => ({
      ...prev,
      [section]: [...prev[section], { key, label: '', detail: '', rationale: '', doctorNote: '', status: 'added' as ItemStatus, isNew: true, phase: 'Phase 1' }],
    }))
    setFlashKey(key); setScrollToKey(key)
  ...
  // ── Save / Approve ───────────────────────────────────────────
  const buildPayload = () => ({
    sections, clinical_impression: clinicalImpression,
    doctor_notes: doctorNotes, rules_version: 'v2.0.0', saved_at: new Date().toISOString(),
  })
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            <textarea
              ...
              rows={5} placeholder="Overall clinical assessment, key findings, treatment rationale…"
              value={clinicalImpression} onChange={e => setClinicalImpression(e.target.value)} disabled={!isEditable} />
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            <textarea
              className="w-full text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-[#538A22] focus:border-transparent placeholder:text-slate-300 disabled:opacity-60 disabled:cursor-not-allowed"
              rows={4} placeholder="Internal notes, follow-up plan, medication interactions…"
              value={doctorNotes} onChange={e => setDoctorNotes(e.target.value)} disabled={!isEditable} />
          </div>
            ...
            <button
              onClick={() => {
                sessionStorage.setItem(`rx_print_${reportId}`, JSON.stringify({
                  sections, clinical_impression: clinicalImpression, doctor_notes: doctorNotes,
                }))
