...
// app/report/[id]/aic-supplements/page.tsx
// AIC Supplement Recommendation Engine — full section page
...
function PhaseBadge({ phase }: { phase: 1 | 2 | 3 }) {
  ...
  const colors: Record<number, string> = {
    ...
    2: 'bg-[var(--pista-50)] text-[var(--pista-700)] border-[var(--pista-200)]',
    ...
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
      ...
      <div className="px-5 py-4 flex items-start justify-between gap-4">
        ...
        <div className="flex-shrink-0 text-right">
          <p className="text-xs font-mono text-[var(--pista-700)] bg-[var(--pista-50)]
            border border-[var(--pista-200)] rounded-lg px-3 py-1.5 whitespace-nowrap">
            {rec.product.timing}
          </p>
          <p className="text-xs text-gray-400 mt-1 text-right">{rec.product.dose}</p>
        </div>
      </div>
          ...
          {rec.triggered_by.map((f, i) => (
            <div key={i} className="flex items-start gap-3 text-xs">
              ...
              <div className="min-w-0">
                ...
                <span className="text-gray-500">{f.observed_value}</span>
      ...
      {/* AI Rationale */}
      {rec.ai_rationale && (
        <div className="mx-5 mb-4 bg-[var(--pista-50)] border border-[var(--pista-100)]
          rounded-xl px-4 py-3">
          <p className="text-xs font-mono text-[var(--pista-600)] uppercase
      ...
      {rec.product.ingredients && rec.product.ingredients.length > 0 && (
        <div className="border-t border-gray-50">
          <button
            onClick={() => setOpen(o => !o)}
            className="w-full px-5 py-2.5 text-left flex items-center justify-between
              text-xs font-mono text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition"
          >
          ...
          {open && (
            <div className="px-5 pb-4">
              <div className="flex flex-wrap gap-1.5">
                {rec.product.ingredients.map((ing, i) => (
                  <span key={i} className="text-xs bg-gray-50 border border-gray-200
      ...
      {/* Note banner */}
      {rec.product.note && (
        <div className="mx-5 mb-4 flex items-start gap-2 text-xs text-amber-700
          bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
  ...
  return (
    <div className="mb-10">
      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg
        text-xs font-mono uppercase tracking-wider mb-1 ${color}`}>
        {title}
      </div>
  ...
  return (
    <div className="bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4 mb-4">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        {months.map((m, i) => (
          <div key={i} className="flex-1 bg-white border border-blue-200 rounded-xl
            px-3 py-2.5 text-xs text-blue-900">
            {m}
          </div>
        ))}
      </div>
...
function ProbioticSchedule({ schedule }: { schedule: string }) {
  return (
    <div className="bg-[var(--pista-50)] border border-[var(--pista-200)]
      rounded-2xl px-5 py-4 mb-4">
      <p className="text-xs font-mono text-[var(--pista-600)] uppercase
        tracking-wider mb-2">
        Probiotic Alternation Schedule
      </p>
      <p className="text-sm text-[var(--pista-900)]">{schedule}</p>
    </div>
  )
}
...
function Warnings({ warnings }: { warnings: string[] }) {
  ...
  return (
    <div className="space-y-2 mb-8">
      {warnings.map((w, i) => (
        <div key={i} className="bg-amber-50 border border-amber-200 rounded-xl
          px-4 py-3 text-xs text-amber-800 leading-relaxed">
          {w}
        </div>
      ))}
    </div>
...
function SummaryStats({ output }: { output: AICRulesOutput }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-3 mb-8">
      {[
        { label: 'Rych Index', value: `${output.rych_index}/100`,
          color: output.rych_index < 40 ? 'text-red-600' : output.rych_index < 60 ? 'text-amber-600' : 'text-[var(--pista-600)]' },
        { label: 'Supplements', value: totalRecs.toString(),
          color: 'text-gray-900' },
        { label: 'Critical', value: critical.toString(),
          color: critical > 0 ? 'text-red-600' : 'text-[var(--pista-600)]' },
      ].map(s => (
...
export default function AICSupplementsPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="w-6 h-6 border-2 border-[var(--pista-500)]
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6">
      ...
      {/* Back link */}
      <Link href={`/report/${id}`}
        className="text-xs text-gray-400 hover:text-[var(--pista-600)]
          transition mb-2 block">
        ...
        <button
          ...
          className={`text-xs font-mono px-4 py-2 rounded-lg border transition flex-shrink-0 ${
            generating
              ...
              : 'bg-[var(--pista-600)] text-white border-[var(--pista-600)] hover:bg-[var(--pista-700)]'
          }`}
        >
          {generating
            ? 'Generating...'
      ...
      {generating && (
        <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center mb-6">
          <div className="w-8 h-8 border-2 border-[var(--pista-500)]
          ...
          <p className="text-xs text-gray-400 mt-1">
            Mapping findings to AIC products and generating clinical rationales
          </p>
        </div>
      )}
      ...
      {!output && !generating && (
        <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center">
          <div className="w-16 h-16 rounded-2xl bg-[var(--pista-50)]
            border border-[var(--pista-200)] flex items-center justify-center
            mx-auto mb-5 text-3xl">
            💊
          </div>
          <h3 className="text-base font-medium text-gray-900 mb-2">
            No supplement plan generated yet
          </h3>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl
              text-sm font-medium text-white bg-[var(--pista-600)]
              hover:bg-[var(--pista-700)] transition"
          >
            Generate Plan
          </button>
        </div>
      ...
      {output && !generating && (
        <>
          {/* Cache indicator */}
          {source === 'cache' && (
            <div className="flex items-center justify-between bg-gray-50 border
              border-gray-200 rounded-xl px-4 py-2.5 text-xs text-gray-400
              font-mono mb-6">
              <span>Showing cached results</span>
              <button
                onClick={() => generate(true)}
                className="text-[var(--pista-600)] hover:text-[var(--pista-700)]"
              >
                Regenerate
              </button>
            </div>
          ...
          <PhaseSection
            ...
            color="bg-[var(--pista-100)] text-[var(--pista-700)] border border-[var(--pista-200)]"
          ...
          <PhaseSection
            ...
            subtitle="Weeks 11–12: Introduce enzyme support once gut lining is healed. Continue core probiotics and nutrients."
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 flex items-center
            justify-between text-xs font-mono text-gray-400">
